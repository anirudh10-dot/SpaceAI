...
class SpaceStationAI:
    ...
    def check_status(self, oxygen_time, water_time):
        ...
        if self.oxygen < 19:
            ...
            status.append("Oxygen dropping")
        
        # CO2
        if self.co2 > 1:
            status.append("CO₂ high خطر")
        elif self.co2 > 0.7:
            status.append("CO₂ rising")
        ...
        if self.humidity < 30 or self.humidity > 70:
            status.append("Humidity imbalance")
...
while True:
    ...
    print(f"Pressure: {ai.pressure:.2f} kPa")
    print(f"Temperature: {ai.temperature:.2f} °C")
    print(f"Humidity: {ai.humidity:.2f}%")
    print(f"Radiation: {ai.radiation:.3f} mSv/hr")
    ...
    print("\nAI Prediction:", ai.predict_future_risk(oxygen_time))
    time.sleep(2)
